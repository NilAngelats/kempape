...
import{getInventory}from"@/lib/server/inventory-game";import{InventoryScreen}from"@/components/inventory-screen";
...
export default async function Section({params}:{params:Promise<{section:string}>}){const {section}=await params;if(!sections[section])notFound();const player=await restoreSession();if(!player)redirect('/login');const bootstrap=await getBootstrap(player);if(bootstrap.lifecycle.status!=='active')redirect('/app');if(section==="actions")return <ActionsScreen model={await getActionsForCurrentPlayer(player)}/>;if(section==="action-pool")return <PoolScreen model={await getActionPool(player)}/>;if(section==="quests")return <QuestsScreen model={await getDailyQuests(player)}/>;if(section==="inventory")return <InventoryScreen model={await getInventory(player)}/>;return <section className="card stack"><span className="badge">Future system</span><h1>{sections[section]}</h1><p>This system will be implemented in a later handoff.</p></section>}
