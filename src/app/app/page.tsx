...
import { getEquippedSlots } from "@/lib/server/inventory-game";
import {getCurrentAppState}from"@/lib/server/app-state";
import{ItemArt}from"@/components/item-art";
export const dynamic = "force-dynamic";

export default async function AppHome(){
  const{session,bootstrap:data}=await getCurrentAppState();if(session.kind!=="valid"||!data)redirect("/login");const player=session.player;
  ...
  return <section className="stack">
    ...
    <section className="card stack"><h2>Equipped</h2><div className="metric-grid">{equipment.slots.map(({slot,item})=><Link href="/app/inventory" className="metric stack compact" key={slot}><span className="muted">{slot[0].toUpperCase()+slot.slice(1)}</span>{item?.definition?<><ItemArt category="equipment" imageKey={item.definition.imageKey} name={item.definition.name} size={120}/><strong>{item.definition.name}</strong></>:<strong>Empty slot</strong>}</Link>)}</div><p className="muted">Standalone slot art is used until approved on-character layers exist.</p></section>
  </section>;
}
