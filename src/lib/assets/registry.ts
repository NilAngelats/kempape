...
const manifestSchema = z.object({ version:z.literal(1), characters:z.record(z.string(),assetSchema), faces:z.record(z.string(),assetSchema), ui:z.record(z.string(),assetSchema), templates:z.record(z.string(),assetSchema), unresolved:z.array(z.string()) });
export const assetManifest = manifestSchema.parse(manifestData);
export type AssetCategory = "characters"|"faces"|"ui"|"templates";
export function getAsset(category:AssetCategory,key:string){return assetManifest[category][key]??null}
export function validateAssetRegistry(existingPaths?:ReadonlySet<string>){const errors:string[]=[];for(const category of ["characters","faces","ui","templates"] as const){const keys=new Set<string>();for(const [key,asset] of Object.entries(assetManifest[category])){if(keys.has(key))errors.push(`Duplicate ${category} key: ${key}`);keys.add(key);if(existingPaths&&!existingPaths.has(asset.path))errors.push(`Missing asset: ${asset.path}`);if(category==="characters"&&asset.assignable===false)errors.push(`Character ${key} cannot be unassignable`);}}return errors;}
