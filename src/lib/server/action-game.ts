...
export async function getActionPool(player:AuthenticatedPlayer){const db=getPrivilegedDatabase(),runId=await getActiveRunId(),now=new Date().toISOString();await db.from("action_submissions").update({status:"expired",resolved_at:now}).eq("game_run_id",runId).eq("status","pending").lte("expires_at",now);const {data,error}=await db.from("action_submissions").select("id,owner_player_id,action_id,submitted_at,expires_at,players!action_submissions_owner_player_id_fkey(display_name,character_id)").eq("game_run_id",runId).eq("status","pending").order("submitted_at");if(error)throw error;return {runId,serverNow:now,entries:(data??[]).map((row:any)=>{const def=ACTIONS.find(v=>v.id===row.action_id);const owned=row.owner_player_id===player.id;return {submissionId:row.id,ownerPlayerId:row.owner_player_id,ownerDisplayName:row.players?.display_name??"Player",ownerFaceAssetKey:null,actionId:row.action_id,actionName:def?.name??row.action_id,tier:def?.tier??"common",submittedAt:row.submitted_at,expiresAt:row.expires_at,remainingValidationTime:Math.max(0,Date.parse(row.expires_at)-Date.now()),canCurrentPlayerAccept:!owned,canCurrentPlayerReject:!owned,ineligibilityReason:owned?"Awaiting validation":null,isOwnedByCurrentPlayer:owned};})};}
